...
void My_gaussian_func(cv::Mat *org_img,cv::Mat *gaussian_image,int size, int sigma,vector<vector<double>> &filter);
double getKernal(int size,int sigma,int x,int y,double z);
int main()
{
    int size;
    double sigma;
    ...
    int intensity1 = org_img.at<uchar>(200, 100); // (200, 100)의 픽셀값

	int intensity2 = org_img.at<uchar>(200, 300); // (200, 300)의 픽셀값

	int intensity3 = org_img.at<uchar>(200, 500); // (200, 500)의 픽셀값
    ...
    int intensity4=org_img.at<cv::Vec3b>(0,1)[2];
     cout <<intensity4<<endl;
    // cout <<org_img.at<cv::Vec3b>(0,1)[0];
    // cout <<org_img.at<cv::Vec3b>(0,3)[2];
    
    cv::GaussianBlur(org_img,gaussian_func_img,cv::Size(),3);
